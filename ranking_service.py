...
def compute_turnover_growth_ratio(*, last_turnover: Optional[float], 
                                  previous_turnover: Optional[float], 
                                  previous_previous_turnover: Optional[float]) -> float:
...
def rank_ngos(financial_df:DataFrameGroupBy) -> List[pd.DataFrame]:
    ...
    for year in financial_df.groups.keys():
        ...
        if merged_df.empty:
            ...
            merged_df = merged_df.merge(group, on="ngo_id", how="outer", suffixes=("", f"_{year}"))
    ...
    for year in years_to_rank:
        ...
        financial_info = financial_info.merge(
            pd.DataFrame({"ngo_id": merged_df["ngo_id"], "growth_ratio": growth_ratios}),
            on="ngo_id",
            how="left",
        )
        ...
        financial_info["growth_rank"] = financial_info["growth_ratio"].apply(growth_rank).astype(int)
        financial_info["balance_rank"] = financial_info["balance_ratio"].apply(balance_rank).astype(int)
        financial_info["stability_rank"] = financial_info["max_income_ratio"].apply(stability_rank).astype(int)

        # Calculate the main rank for each year
        financial_info["main_rank"] = (
            0.4 * financial_info["growth_rank"] + 0.4 * financial_info["balance_rank"] + 0.2 *
            financial_info["stability_rank"]).astype(int)
        ...
        financial_info["percentile_num"] = np.ceil(grouped_df["main_rank"].transform('rank', pct=True).values / 0.2).astype(int)
        financial_info["percentile_label"] = financial_info["percentile_num"].apply(percentile_label)

        # Add the means for each ratio for each turnover category
        financial_info["admin_expense_benchmark"] = grouped_df["admin_expense_ratio"].transform('mean')
        financial_info["growth_benchmark"] = grouped_df["growth_ratio"].transform('mean')
        financial_info["balance_benchmark"] = grouped_df["balance_ratio"].transform('mean')
        financial_info["max_income_benchmark"] = grouped_df["max_income_ratio"].transform('mean')
        # Add the mean for the main rank for each turnover category
        financial_info["main_rank_benchmark"] = grouped_df["main_rank"].transform('mean')
    ...
    for idx,  financial_info in enumerate(financial_infos[:len(financial_infos) - 1]):
        ...
        financial_info[f"main_rank{year_diff_suffix}"] = financial_info["main_rank"] - previous_financial_info["main_rank"]
        financial_info[f"growth_rank{year_diff_suffix}"] = financial_info["growth_rank"] - previous_financial_info["growth_rank"]
        financial_info[f"balance_rank{year_diff_suffix}"] = financial_info["balance_rank"] - previous_financial_info["balance_rank"]
        financial_info[f"stability_rank{year_diff_suffix}"] = financial_info["stability_rank"] - previous_financial_info["stability_rank"]
