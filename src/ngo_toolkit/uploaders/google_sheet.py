...
from ngo_toolkit.scrapers.cfi_midot_scrapy.items import OrderedSchema
import logging
...
def load_all_ranked_years() -> List[list]:
    ...
    ranked_years = []
    ranked_files = glob(f"data/{settings.RANKED_NGO_SHEET_NAME}_*.csv")
...
def upload_spread_sheet(
    ...
    for year, ranks in publish_sheet_values.items():
        create_sheet_if_not_exists(credentials, settings.PUBLIC_SPREADSHEET_ID, prefix, year)
        write_to_sheet(credentials, settings.PUBLIC_SPREADSHEET_ID, f"{prefix}{year}", ranks)
